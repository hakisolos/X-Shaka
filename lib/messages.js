...
const serialize = async (_msg, conn) => {
    if (_msg.key) {
        ...
        _msg.isowner = _msg.sender === conn.user.id;
    }
    ...
    if (_msg.message) {
        ...
        _msg.reply = (text) => client.sendMessage(_msg.user, { text }, { quoted: _msg });
        _msg.mentions = [];
        ...
        try {
            await conn.sendMessage(_msg.user, {
                react: { text: emoji, messageId: _msg.id }
            });
        } catch (error) {}
    };
