...
async function serialize(msg, conn) {
    if (msg.key) {
        ...
        m.device = /^3A/.test(msg.id) ? 'ios' : msg.id.startsWith('3EB') ? 'web' : /^.{21}/.test(msg.id) ? 'android' : /^.{18}/.test(msg.id) ? 'desktop' : 'unknown';
        m.isbot = msg.id.startsWith('BAE5') || msg.id.startsWith('XAT');
