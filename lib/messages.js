...
const downloadMedia = async (message) => {
    try {
        ...
        let mediaMessage = message[messageType];
        if (messageType === 'buttonsMessage' || messageType === 'viewOnceMessageV2') {
            if (messageType === 'viewOnceMessageV2') {
                ...
                const nestedType = Object.keys(mediaMessage || {})[0];
                mediaMessage = mediaMessage[nestedType];
            } else {
                const nestedType = Object.keys(mediaMessage || {})[1];
                mediaMessage = mediaMessage[nestedType];
            }
        }

       if (!mediaMessage || !mediaMessage.url || !mediaMessage.mediaKey) {
            throw new Error('Media content is missing required properties');
        }
