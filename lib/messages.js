    ...
    .filter(a => a) || []; 
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.cMod = function (user, text = '', sender = jidNormalizedUser(conn?.user?.id), options = {}) {
					return conn.cMod(user, m.msg?.contextInfo?.quotedMessage, text, sender, options);
				};
			}
		}
	}

	m.reply = m.user ? async (text, opts = {}) => await conn.sendMessage(m.user, { text, ...opts }, { quoted: m }) : null;
	m.copy = m.message ? conn.cMod.bind(conn, m.user, m) : null;
