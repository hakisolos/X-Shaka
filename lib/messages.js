...
const downloadMedia = async (message) => {
    if (!message || !message.message) {
        throw new Error('No_found_imao');}
    var messagis = Object.keys(message.message)[0];
    if (messagis === 'viewOnceMessageV2') {
        message = message.message.viewOnceMessageV2.message;
        messagis = Object.keys(message)[0];}
    const suppor = ['stickerMessage', 'imageMessage', 'videoMessage', 'documentMessage', 'audioMessage'];
    if (!suppor.includes(messagis)) {
    throw new Error(`${messagis}`);}
    const medi = message[messagis];
    const media_ = messagis.replace('Message', '').toLowerCase();
    const stream = await downloadContentFromMessage(medi, media_);
