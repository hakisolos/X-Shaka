...
const decodeJid = (jid) => {
    ...
    try {
        ...
        return decoded ? `${decoded.user || ''}@${decoded.server || ''}`.trim() : jid;
    } catch (err) {
        console.error(err, jid);
        return jid;
    }
};
...
async function fetchGroupAdmins(conn, groupId) {
    ...
    return metadata.participants.filter((member) => member.admin).map((member) => decodeJid(member.id));
}
...
async function serialize(msg, conn) {
    ...
    if (msg.message) {
        ...
        msg.forward = (jid, options = {}) =>
            msg.send(jid, { text: msg.text }, { quoted: null, ...options });
        msg.mentions = [];
        ...
        msg.react = async (reaction) => {
            if (msg.key) {
                await conn.sendMessage(msg.user, { react: { text: reaction, key: msg.key } });
            } else {
    ...
    if (msg.isGroup) {
        ...
        msg.isBotAdmin = admins.includes(decodeJid(conn.user.id));
    }
