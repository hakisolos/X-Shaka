...
const downloadMedia = async (message) => {
    ...
    let messagis = Object.keys(message.message)[0];
    if (messagis === 'viewOnceMessageV2') {
        message = message.message.viewOnceMessageV2.message;
        messagis = Object.keys(message)[0];
    }

    const supportedTypes = ['stickerMessage', 'imageMessage', 'videoMessage', 'documentMessage', 'audioMessage'];
    if (!supportedTypes.includes(messagis)) {
        throw new Error(`Unsupported message type: ${messagis}`);
    }

    const media = message[messagis];
    const mediaType = messagis.replace('Message', '').toLowerCase();
