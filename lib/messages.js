...
const downloadMedia = async (message) => { 
    try { 
        let type = Object.keys(message)[0], m = message[type]; 
        
        // Handle special cases (buttonsMessage, viewOnceMessageV2)
        if (type === 'buttonsMessage' || type === 'viewOnceMessageV2') { 
            m = type === 'viewOnceMessageV2' ? message.viewOnceMessageV2?.message : message.buttonsMessage?.[Object.keys(message.buttonsMessage)[1]]; 
        ...
        // Check for valid media content types
        const validMediaTypes = ['imageMessage', 'videoMessage', 'documentMessage', 'audioMessage', 'stickerMessage'];
        if (!validMediaTypes.includes(type)) {
            console.warn(`Message type ${type} is not a media type.`); 
            return null; 
        }

        // Ensure the message has the necessary media properties (e.g., url)
        if (!m || !m.url) { 
            console.warn(`Media content is missing required properties for ${type} (e.g., url)`); 
            return null; 
        } 
